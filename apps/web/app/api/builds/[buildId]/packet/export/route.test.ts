...
function get(buildId: string, search = "") {
  return GET(new Request(`http://localhost/api/builds/${buildId}/packet/export${search}`), {
...
describe("packet export route", () => {
  ...
  it("returns 400 for an unsupported format", async () => {
    ...
    const response = await get(build.id, "?format=pdf");
    expect(response.status).toBe(400);
  });

  it("defaults to markdown with approved content and excludes drafts", async () => {
    mockedGetSession.mockResolvedValue({ userId: "user-a" });
    const build = await ownedBuild("user-a");
    ...
    expect(response.headers.get("Content-Disposition")).toBe(
      `attachment; filename="launch-packet-${build.id}.md"`,
    );

    const body = await response.text();
    expect(body).toContain("# Launch packet");
    expect(body).toContain("Creator-economy tax tooling is underserved");
    expect(body).not.toContain("Draft headline pending review");
  });

  it("returns json artifact excluding draft content", async () => {
    mockedGetSession.mockResolvedValue({ userId: "user-a" });
    const build = await ownedBuild("user-a");

    const response = await get(build.id, "?format=json");
    expect(response.status).toBe(200);
    expect(response.headers.get("Content-Type")).toBe("application/json");
    expect(response.headers.get("Content-Disposition")).toBe(
      `attachment; filename="launch-packet-${build.id}.json"`,
    );

    const data = (await response.json()) as {
      formatVersion: number;
      buildId: string;
      sections: Array<{ key: string; content: unknown }>;
    };
    expect(data.formatVersion).toBe(1);
    expect(data.buildId).toBe(build.id);
    expect(data.sections).toHaveLength(5);
    expect(data.sections.find((section) => section.key === "copy-deck")?.content).toBeNull();
  });
});
