...
// Survive Next.js dev/HMR module reloads so a build created via the API is
// still readable by the subsequent server render of the session page (same process).
 ...
 * - Otherwise an in-memory singleton so the create → redirect → server-render
 *   flow works locally, in CI, and in e2e without a database.
 */
